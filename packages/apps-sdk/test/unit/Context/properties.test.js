...
test('propertyProviderTab rejects with error if tabId not passed via Context', () => {
  ...
  return propertyProviderTab(outgoingDispatcher, contextProps)('data.me').catch(e => {
    expect(e instanceof Error).toEqual(true);
    expect(e.message).toEqual('missing tab id from context, property fetching will not work');
  });
});
