...
class DeskproSDK extends React.Component {
  ...
  handleDisplayChange = () => {
    // force a re-render
    this.setState({
      uiChangeRequests: this.state.uiChangeRequests + 1,
    });
  };
  ...
  renderErrors = () => {
    ...
    return sdk.errors.map(error => (
      <div
        key={error.id}
        className="dp-alert dp-alert--danger dp-bg--danger"
        style={{ margin: 10 }}
      >
        <div className="dp-alert__content">{error.msg}</div>
        <div
          title="Close"
  ...
  renderApp = () => {
    ...
    if (component && children) {
      throw new Error(
        'You can have either the App as a property or as a child',
      );
    }
  ...
  render() {
    ...
    return (
      <AppFrame
        badgeText={
          dpapp.ui.badge === UIConstants.VISIBILITY_VISIBLE
            ? dpapp.ui.badgeCount
            : null
        }
        ...
        actions={[
          ...
          <Action
            icon={dpapp.ui.isCollapsed() ? 'down' : 'up'}
            onClick={
              dpapp.ui.isCollapsed() ? dpapp.ui.expand : dpapp.ui.collapse
            }
          />,
        ]}
      >
        {this.renderErrors()}
        {!sdk.readyForApp ? this.renderLoading() : this.renderApp()}
      </AppFrame>
    );
  }
}
