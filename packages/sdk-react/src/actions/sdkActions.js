...
export function bootstrap() {
  return function(dispatch, getState, dpapp) {
    ...
    const promises = [
      ...
      .concat(oauthKeys.map(key => dispatch(oauthGetSettings(key))))
      .filter(p => !!p);
...
export function me() {
  return function(dispatch, getStorage, dpapp) {
    return dpapp.restApi.get('/me').then(resp => {
      try {
...
export function appGetStorage(key, defaultValue = null, cb = () => {}) {
  ...
  return (dispatch, getState, { storage }) => {
    return storage
      .getAppStorage(key, defaultValue)
      .then(value => {
...
export function entityGetStorage(key, defaultValue = null, cb = () => {}) {
  ...
  return (dispatch, getState, { storage }) => {
    return storage
      .getEntityStorage(key, defaultValue)
      .then(value => {
        if (Array.isArray(key)) {
